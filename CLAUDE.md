...
The `meta` plugin's improvement loop uses `~/.claude/skill-sessions/<plugin>/sessions/<skill-name>/` (outside the repo) to store session logs — this allows `/skill-reflect` to run in any project and accumulate logs that `/skill-improve` reads when run from this repo.
